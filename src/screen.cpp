...
vita2d_texture* getCopy(vita2d_texture *texture, float x, float y, float width, float height) {
	...
	int* dest = (int*)vita2d_texture_get_datap(copy), *data = (int*)vita2d_texture_get_datap(texture);
	int pitch = vita2d_texture_get_stride(texture) >> 2, petch = vita2d_texture_get_stride(copy) >> 2;
	for (int i = x, k = 0; i < x + width; i++, k++)
		for (int j = y, n = 0; j < y + height; j++, n++)
			dest[k + n * petch] = data[i + j * pitch];
	return copy;
};
vita2d_texture* make_screenshot() {
	vita2d_texture* copy = vita2d_create_empty_texture(1024, 544);
...
game_screen::game_screen(game *owner) : screen(owner) {
	...
	for (int i = 0; i < records.length(); i++) {
		if (records[i] == ' ') {
			n++;
			if (n == 1)	record_e.Set(stoi(now_rec));
			if (n == 2)	record_m.Set(stoi(now_rec));
			if (n == 3)	record_h.Set(stoi(now_rec));
			if (n == 4)	record_c.Set(stoi(now_rec));
			if (n == 5) break;
			now_rec = "";
		}
		else {
...
void pause_screen::draw() {
	if (y != 0 || owner->state==2) {
		if (owner->gamescreen->created) {
			...
			owner->font->write(950, 522, 1, "creckeryop 2019", RGBA8(255, 255, 255, 255), FONT_RIGHTED);
	...
	if (x != 0) {
		if (owner->gamescreen->created) {
			for (int i = 0, y = 0; i < 6; i++, y += 46) {
				...
				if (i == 1)	now = states[i] + "<" + fillZero(owner->width, 2) + ">";
				if (i == 2)	now = states[i] + "<" + fillZero(owner->height, 2) + ">";
				if (i == 3)	now = states[i] + "<" + fillZero(owner->mines, 4) + ">";
				if (i == 4)	now = states[i] + "<" + to_string(owner->theme) + ">";
				if (i == 5)	now = states[i];
...
void settings_screen::control(SceCtrlData &pad, SceCtrlData &oldpad) {
	if (pad.buttons & SCE_CTRL_UP && !(oldpad.buttons & SCE_CTRL_UP)) if (--state < 0)	state = 5;
	if (pad.buttons & SCE_CTRL_DOWN && !(oldpad.buttons & SCE_CTRL_DOWN))	if (++state > 5) state = 0;
	...
	if (pad.buttons & SCE_CTRL_LEFT || pad.buttons & SCE_CTRL_RIGHT) {
		if (pause.GetTime() > key_pause || key_pause == pause_1) {
			...
			if (pad.buttons & SCE_CTRL_RIGHT) {
				switch (state) {
					case 0:	if (++owner->mode >= 4)	owner->mode = 0; break;
					case 1:	if (++owner->width > 40) owner->width = 40; break;
					case 2: if (++owner->height > 40) owner->height = 40; break;
					case 3: if (++owner->mines > 9999) owner->mines = 9999; break;
					case 4: 
			...
			if (pad.buttons & SCE_CTRL_LEFT) {
				switch (state) {
					case 0: if (--owner->mode < 0)	owner->mode = 3; break;
					case 1:	if (--owner->width < 5)	owner->width = 5; break;
					case 2:	if (--owner->height < 5) owner->height = 5;	break;
					case 3:	if (--owner->mines < 5)	owner->mines = 5; break;
					case 4: 
						((game_screen*)owner->gamescreen)->change_theme(owner->theme - 1);
						((game_screen*)owner->gamescreen)->save();
						break;
