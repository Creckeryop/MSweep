...
std::string fillZero(int num, int max) {
	std::string str = "";
...
game_screen::game_screen(game *owner) : screen(owner) {
	...
	record_c.Reset();
	first = true;
	tile1 = vita2d_load_PNG_file("app0:assets/grass1.png");
	tile2 = vita2d_load_PNG_file("app0:assets/grass2.png");
	...
	if (fs >= 0) {
		char *lol = new char[1000];
		sceIoRead(fs, lol, size);
		records = std::string(lol);
		sceIoClose(fs);
	}
	std::string now_rec = "";
	int n = 0;
	for (int i = 0; i < records.length(); i++) {
		if (records[i] == ' ') {
			n++;
			if (n == 1)
				record_e.Set(StringToInt(now_rec));
			if (n == 2)
				record_m.Set(StringToInt(now_rec));
			if (n == 3)
				record_h.Set(StringToInt(now_rec));
			if (n == 4)
				record_c.Set(StringToInt(now_rec));
			now_rec = "";
		}
		else {
...
void game_screen::open() {
	...
	bool check = false;
	if (scene->actField[frame_x][frame_y] <= 1) {
		check = true;
	...
	if (state == 0 && scene->state == 1) {
		time.Pause();
		bomb.Reset();
		for (int i = 0; i < scene->width; ++i)
			for (int j = 0; j < scene->height; ++j)
				if (scene->minField[i][j])
					scene->actField[i][j] = 1;
	}
	if (state == 0 && scene->state == 2) {
		time.Pause();
		...
		if (record->GetTime() == 0 || record->GetTime() > time.GetTime()) {
			...
			if (fs >= 0) {
				std::string rec = std::to_string(record_e.GetTime()) + " " + std::to_string(record_m.GetTime()) + " " + std::to_string(record_h.GetTime()) + " " + std::to_string(record_c.GetTime());
...
void game_screen::control(SceCtrlData &pad, SceCtrlData &oldpad) {
	...
	if (pad.buttons & SCE_CTRL_START && !(oldpad.buttons & SCE_CTRL_START) && owner->pausescreen->y==0) {
		...
		lol = vita2d_create_empty_texture(1024, 544);
		created = true;
		unsigned int *buffer = (unsigned int*)vita2d_get_current_fb();
		int* data = (int*)vita2d_texture_get_datap(lol);
		for (int i = 0; i < 544 * 1024; i++) {
			data[i] = buffer[i];
		}
	}
}
...
void game_screen::draw() {
	if (scene != NULL) {
		if (!created) {
			vita2d_draw_rectangle(0, 0, 960, 544, COLOR_BACKGROUND);
			float width = 17 * size;
			...
			for (int i = x_s - 1; i >= s_x; i--) {
				...
				for (int j = s_y; j < y_s; j++) {
					if (scene->actField[i][j] == 1) {
						if (i < scene->width - 1 && scene->actField[i + 1][j] == 1)
							vita2d_draw_rectangle(X + width, Y + size, size, width - size, COLOR_TILE_SHADOW);
						if (j > 0 && scene->actField[i][j - 1] == 1)
							vita2d_draw_rectangle(X, Y, width, size, COLOR_TILE_SHADOW);
						if (scene->numField[i][j])
							vita2d_draw_texture_part_tint_scale_rotate(font, X + (int)(width / 2), Y + (int)(width / 2), scene->numField[i][j] * 8, 0, 8, 9, size, size, 0, Colors[scene->numField[i][j]]);
						if (scene->minField[i][j])
							bomb.Play(X + width / 2, Y + width / 2, 0);
					}
					else {
						vita2d_draw_texture_scale((i % 2 == j % 2) ? tile1 : tile2, X, Y, size, size);
						if (j < scene->height - 1 && scene->actField[i][j + 1] == 1)
							vita2d_draw_rectangle(X, Y + width + size, width, size, COLOR_TILE_SHADOW);
						if (i > 0 && scene->actField[i - 1][j] == 1)
						...
						if (scene->actField[i][j] == 2)
							...
							if (scene->actField[i][j] == 3)
								mark.Play(X + width / 2, Y + width / 2, 0);
						if (scene->state == 0 && (hold_x >= 0 && hold_y >= 0 && scene->actField[hold_x][hold_y] == 1 && around(i, j) && scene->actField[i][j] == 0 || hold_x == i && hold_y == j))
			...
			vita2d_draw_texture_scale(frame, p_x + frame_x * width, p_y + frame_y * width, size, size);
			...
			vita2d_draw_texture_scale(mine, 10, 72, 2, 2);
			owner->font->write(45, 74, 2, std::to_string(scene->mines - scene->flags), RGBA8(0, 0, 0, 60));
			owner->font->write(48, 71, 2, std::to_string(scene->mines - scene->flags), RGBA8(255, 255, 255, 255));
...
void game_screen::update(double dt) {
	if (!created) {
		int vec_x = 480 - p_x - frame_x * 17 * size - 17 * 0.5 * size, vec_y = 272 - p_y - frame_y * 17 * size - 17 * 0.5 * size;
		p_x += vec_x / 8;
		p_y += vec_y / 8;
	}
	std::vector<std::vector<particle>::iterator> to_delete;
	for (std::vector<particle>::iterator i = particles.begin(); i < particles.end(); i++) {
		(*i).Update(dt);
		if ((*i).y + p_y > 544)
...
void pause_screen::draw() {
	if (owner->gamescreen->created) {
		for (int i = 0, x = 32; i < 4; i++, x+=32) {
			owner->font->write(x, 544/2 - 45/2 + owner->settingscreen->y, 3, states[i], state == i ? RGBA8(255, 255, 255, 255) : RGBA8(100, 100, 100, 255));
			x += states[i].length() * 24;
		}
		vita2d_draw_rectangle(0, -y, 960, 544 / 2, RGBA8(0, 0, 0, 255));
		vita2d_draw_rectangle(0, 544/2 + y, 960, 544 / 2, RGBA8(0, 0, 0, 255));
		vita2d_draw_texture_part(owner->gamescreen->lol, 0, -y, 0, 0, 960, 544 / 2);
		vita2d_draw_texture_part(owner->gamescreen->lol, 0, 544 / 2 + y, 0, 544/2 , 960, 544 / 2);
	}
}
void pause_screen::update(double dt) {
	...
	if (owner->state >= 2) {
		y = EaseOutCubic(t.GetTime(), 0, 44, 200);
	}
	else {
		y = EaseOutCubic(t.GetTime(), 44, -44, 200);
	}
}
...
void settings_screen::draw() {
	if (owner->gamescreen->created) {
		for (int i = 0, x = 120; i < 4; i++, x += 32) {
			std::string now = modes[owner->mode];
			if (i==1)
			...
			owner->font->write(x, 544 / 2 + 30 + 25 + y, 2, now, state == i ? RGBA8(255, 255, 255, 255) : RGBA8(100, 100, 100, 255));
			if (i == 0)
				x += 6 * 16;
			else
				x += now.length() * 16;
...
void settings_screen::update(double dt) {
	...
	if (owner->state == 3) {
		y = EaseOutCubic(t.GetTime(), 0, -70, 200);
	}
	else {
		y = EaseOutCubic(t.GetTime(), -70, 70, 200);
	}
}
void settings_screen::control(SceCtrlData &pad, SceCtrlData &oldpad) {
	...
	if (pad.buttons & SCE_CTRL_UP || pad.buttons & SCE_CTRL_DOWN) {
		if (pause.GetTime() > key_pause || key_pause == pause_1) {
			pause.Reset();
			if (state==0)
				key_pause = pause_2;
